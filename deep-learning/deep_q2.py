...
vol = modal.Volume.from_name("long_run", create_if_missing=True)
...
@app.function(gpu="A100", image=image, timeout=86400, volumes={"/data": vol})
def train_rl(env_name: str = "spades", algorithm: str = "dqn", num_episodes: int = 5000, seed: int = 42,num_eval_games: int = 2000, evaluate_every:int=100, log_directory:str = "experiments/testing-a100-2/" ):
    ...
    with Logger('/data/'+log_directory) as logger:
        ...
        for episode in tqdm(range(num_episodes)):
            ...
            if episode % evaluate_every == 0:
                ...
                #vol.commit()
        model_filename = f"/data/models/model_{env_name}_a1002_{algorithm}_{num_episodes}ep.pt"
...
def main(env_name: str = "spades", algorithm: str = "dqn", episodes: int = 1000000, seed: int = 42, evals: int = 500, evaluate:int=100, log:str = "experiments/testing-a100-2/" ):
